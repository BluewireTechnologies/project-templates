...
if (!$$safeprojectname$Principal) { throw "`$$safeprojectname$Principal was not specified." }
$fullyQualifiedPrincipalName = Resolve-FullyQualifiedPrincipalName "${$safeprojectname$Principal}";

schtasks.exe /Create /TN "$projectname$" /XML "${thisDir}\TaskDefinition-Compatible.xml" /RU "${$safeprojectname$Principal}" /RP "${$safeprojectname$Password}" /F
